...
df = pd.read_csv("D:\S\start\code\seeds\\firstdata\\triple\\yongyou9_train.csv",  header=None)
train_targets = df.values[:,224]  #? 这里修改为224
train_data = df.values[0:600,19:200]  # 扫描的数据中的行 列
#print(train_targets)
df2 = pd.read_csv("D:\S\start\code\seeds\\firstdata\\triple\\yongyou9_test.csv",  header=None)
test_targets = df2.values[:,224]  #? 这里修改为224
test_data = df2.values[0:100,19:200]
#print(df2.head(5))

df3 = pd.read_csv("D:\S\start\code\seeds\\firstdata\\triple\\yongyou9_val.csv", header=None)
...
class CNN(nn.Module):
    def __init__(self,):
        ...
        self.gru = nn.GRU(input_size=32, hidden_size=50, num_layers=1, batch_first=True)
