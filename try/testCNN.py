...
df = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_weilaohua_train.csv",  header=None)
train_targets = df.values[:,224]  #? 这里修改为224
train_data = df.values[0:400,19:200]  # 扫描的数据中的行 列
#print(train_targets)
df2 = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_weilaohua_test.csv",  header=None)
test_targets = df2.values[:,224]  #? 这里修改为224
test_data = df2.values[0:200,19:200]
#print(df2.head(5))

df3 = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_weilaohua_val.csv", header=None)
...
class CNN(nn.Module):
    def __init__(self,):
        ...
        self.downsample1 = nn.Conv1d(1, 8, kernel_size=1)
        ...
        self.downsample2 = nn.Conv1d(8, 16, kernel_size=1)
        ...
        self.downsample3 = nn.Conv1d(16, 32, kernel_size=1)
...
# 定义优化器
# ! 过拟合时修改weight_decay
optimizer = torch.optim.Adam(zh.parameters(),lr=learning_rate,weight_decay= 1e-4)
