...
file_dir = r"E:\8.24first\SZY" ##########################
saveGrayPhotodir =  "D:\\S\\start\\code\\getHy\\photo"##########
saveHydir = 'D:\\S\\start\\code\\getHy'##############
filePath_list = []
previousRoundHdr = None

for epoch in range(0,8): #####################
    s = '{}-{}'.format(seedName,epoch+1)
    ...
    for i in filePath_list:
        if os.path.splitext(i)[1] == ".hdr" and ("REFLECTANCE" in i or "refl" in i):  # 筛选文件
            hdrFile = i
            print("第{}轮.hdr文件为：".format(epoch+1),hdrFile)
        if os.path.splitext(i)[1] == ".dat":  # 筛选文件
            datFile = i
            print("第{}轮.dat文件为：".format(epoch+1),datFile)
        if os.path.splitext(i)[1] == ".png":  # 筛选文件
            rgbFile = i
            print("第{}轮.png文件为：".format(epoch+1),rgbFile)
    ...
    # print('labels = ',labels)
    np.savetxt('D:\S\start\code\getHy\labels\labels.csv', labels, fmt='%.4f', delimiter=',')
    ...
    for x in zipped_10:
        ...
        for j in range(10):
            ...
            z = z + 1
    # print(list_sortted)
    fi = open("D:\\S\\start\\code\\getHy\\labels\\labels.csv", 'r', encoding="utf-8")
    ...
    np.savetxt('{}\\data\\{}.csv'.format(saveHydir,s,), xiangsu, fmt='%.4f', delimiter=',')
