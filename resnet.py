...
df = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_laohua192h_train.csv",  header=None)
train_targets = df.values[:,224]  #? 这里修改为224
train_data = df.values[0:400,19:200]  # 扫描的数据中的行 列
#print(train_targets)
df2 = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_laohua192h_test.csv",  header=None)
test_targets = df2.values[:,224]  #? 这里修改为224
test_data = df2.values[0:200,19:200]
#print(df2.head(5))

df3 = pd.read_csv("D:\S\start\code\seeds\Binary classification data\data\longjingyou_laohua192h_val.csv", header=None)
